...
@test "checking PERMIT_DOCKER: connected-networks" {
  ...
  # The relayed mail cannot actually deliver to the fake external.tld domain, a
  # delivery attempt in the log is proof the relay was permitted (not rejected):
  repeat_in_container_until_success_or_timeout 60 mail_smtponly_second_network /bin/sh -c 'grep -cE "to=<user2\@external.tld>" /var/log/mail/mail.log'
}
